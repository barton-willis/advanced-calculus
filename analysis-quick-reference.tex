...
\begin{minipage}[c]{0.1665\textwidth}
    \begin{tabular}{|L | L|} \hline
        \mbox{Meaning}  & \mbox{Symbol} \\ \hline
        \mathrm{union} & \cup  \\ 
        \mathrm{complment} & \mbox{superscript}^\mathrm{C} \\
...
\section*{Elementary function properties}
    \begin{description}[\itemsep=0em]
        ...
        \item[Bounded below] \(\left (\exists \, M \in \reals \right) \parens*{\forall x \in \dom(F)} \parens*{M < F(x)}\)
    \end{description}
