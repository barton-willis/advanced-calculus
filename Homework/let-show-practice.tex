...
\begin{parts}
    ...
    \part For all $x,y \in \reals$, there is $a \in \reals$ such that
    $x < y$ implies $x<a<y$
    ...
    \end{equation*}
     This  statement says that between any two real numbers, there is 
    a real number. One such number is the arithmetic average. So   let's write a proof.
    But for completeness, its negation is
    \begin{equation*}
    ...
    \begin{proof}
     ...
     We have $a \in \reals$ as required. We have
     \begin{align*}
       \left[ x < a < y \right] 
     ...
     \end{align*}
     A key ingredient to the proof is the fact that $(\forall a,x,y \in \reals)( (x< y) \equiv (x-a < y-a))$.      \hfill (BW) 
      \end{proof}
     
     \part For all $r \in \reals_{>0}$ there is $x \in [0,1)$ such that $1-r < x$.      \hfill (BW) 
     
     \part For all $x \in \reals_{>0}$ there is $y \in \reals_{> 0}$ such that $y < x$. \hfill (BW) 
     
     \part There is $y \in \reals_{> 0}$ such that for all $x \in \reals_{>0}$ we have $y < x$.
