...
The part with the boolean value is called the \emph{predicate}. Expressing the proposition in symbolic form gives us a road map 
for constructing  a proof. To follow the road map, we need to strictly proceed from left to right. Additionally, to successfully follow the road map,
 we to adhere to some rules and follow some traditions.  Following the rules is needed for correctness,  and using the  traditions makes 
...
\begin{myproof} (BW)
    ...
    \end{align*} 
\end{myproof}
\noindent Each step in this series has a parenthetical remark clarifying the reason for the equivalence.  I encourage you all to 
clarify your work this way. Also, if the fact that the arithmetic average is known to be ``in the middle,'' our proof could end at 
the second line. It's sometimes a judgment call on whether or not a fact can be used in a proof. When in doubt, include, don't assume.
...
logical implications. If we expressed this work as
 \begin{align*}
    ...
    \end{align*} 
we have shown that  $(x < a < y) \implies \true$.  But $P \implies \true$ is a tautology, so $P \implies \true$ is
true \emph{regardless} of the truth value of $P$. So the above work proves \emph{nothing}. By reversing the order of the statements,
