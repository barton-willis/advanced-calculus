...
Our proof breaks into two cases:
      ...
      \end{align*}
      \end{myproof}
 \noindent If you prefer to condense this proof into one case, try choosing $r = \max(\frac{1}{2}, 1-\frac{1}{2})$.
...
\end{myproof}
  ...
  \end{align*} 
    ...
    \end{myproof}
     ...
     \end{prop}
      ...
      A good way of thinking about this proposition is geometrically as two graphs. In the x-y plane, the graph of  $y = 1 + m(x-1)$, is a line  and the
  ...
  \end{myproof}
    ...
    is a counterexample to the proposition, for  practice negating qualified statements and conditionals,  we'll explicitly negate the proposition and write a let/show/suppose  proof that it's negation is true.
...
\begin{align*}
 \left[x^3 = y^3 \right] &\equiv \left [x^3 - y^2 = 0 \right],   &&(\mbox{algebra}) \\
                                        ...
                                        &\equiv \left [(x-y) = 0   \lor  (x + y/2)^2 + 3 y^2 /4)  = 0 \right],   &&(\text{ordered field properties}) \\
                                         ...
                                         &\equiv \left[x=y\right].  &&(\text{disjunction condense})
\end{align*}
\end{myproof}
