...
\item[\#1] Write the proposition in the form of a question:
 ...
 \textbf{Answer} Given a real number \(x\), can I find a number \(y\) such that \(x < y\)?

\item[\#2] Answer your question.  

 \textbf{Answer} Sure--a number that is greater than \(x\) is \(x +1\).
...
\vspace{0.1in}
\textbf{Question} Given a positive  real number \(x\), can I find a positive  \(y\) such that \(y\) is smaller than \(x\)?

\item[\#2] Answer your question.  

\vspace{0.1in}
\quad  \textbf{Answer} Sure--a positive number that is smaller than \(x\) is the average of zero and \(x\); that is \(x/2\).


\item[\#3] I'm ready, I think:
