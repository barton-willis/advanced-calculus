...
\[
F = x \in  (-\infty, \infty)  \mapsto \begin{cases}  \mbox{true} & \mbox{ if } x \mbox{ is rational } \\   \mbox{false} & \mbox{ if } x \mbox{ is irrational } \end{cases}
\]
is a predicate. We have, for example
\[
    F(2/3) = \mbox{true},  \quad F(\sqrt{2})  = \mbox{false},  \quad F(\uppi)  = \mbox{false}, \quad F(\mathrm{e})  = \mbox{false},
\]
...
\begin{frame}{Set \part{predicates}} Let \(A\) and \(B\) be sets. Define
\begin{align*}
