...
\usepackage{fullpage,amsmath,pifont,moreverb,enumerate}
...
%\author{Barton Willis \\ Professor of Mathematics}
...
correct then is a long proof that starts from scratch.  Whenever
...
you'll agree that the method doesn't work.  If you don't throughly
understand the definition of every word in the statement of a theorem,
you won't be able to prove it.  When you throughly understand a
...
\(\star\) as  \(x = 5 \) instead; \((\star\)) implies {\em two\/}
facts
\[
  x < 5 \mbox{ and } x < 6.
\]
...
\]
Given a {\em specific\/} value for \(\varepsilon\), we could reduce
these two inequalities into just one; for example, if \(\varepsilon =
10\), we'd simply have \(x < 1\). And if if \(\varepsilon =
