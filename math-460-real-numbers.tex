...
\]
It has an \emph{left identity element}  provided
\[
  (\exists\,\, \theta \in S)((\forall a \in S)(F(\theta,a) = a).
\]
And it has an \emph{right identity element}  provided
\[
...
\item Addition and multiplication of real numbers are examples of binary operators; these operators are commutative and associative.

\item In this context, binary means that the function takes \emph{two} members of the same set; the use of binary has nothing to do with base two representation of a number.

\item Usually binary operators are expressed in \emph{infix notation}; that is, the operator is in between its arguments.

\item For example, we write \(1 + 107 = 108\), not \(+(1,107) = 108\).

\item  For a commutative binary operator,  every right identity element is a left identity element; so we'll call them collectively an identity element.
\end{enumerate}
\end{frame}
...
\item Addition \(+\) is a binary operator on \(\reals\).  Since \(x +0 = x\) for all real \(x\), the unit for addition is zero.  Further we know that addition is
commutative and associative.

\item Function composition \(\circ \) is a binary operator on the set of functions from \(\reals\) to \(\reals\).  The function \(x \in \reals \mapsto x\) is the unit for
function composition .  Function composition is associative, but not commutative.
...
\begin{myth} Let \( S \) be a set and let \(F\) be a commutative binary operator on \(S\).  Then \(F\) has at most one identity element.

 \end{myth}

 \begin{myproof} Let \(\theta\) and \(\theta^\prime\) be identity element. for \(F\). We'll show that \(\theta = \theta^\prime\).  We have
 \begin{align*}
...
We would like to capture the important features of the real numbers and give all such structures a name. This object is a \emph{field}.

\begin{mydef}A field is an ordered triple  \(({\mathcal F},  +, \times)\) where \({\mathcal F} \) is a set and both \(+\) and \(\times\) are commutative and associative binary operators on
\({\mathcal F}\) that have  identity elements; the  identity element for \(+\) is 0 and the  identity element  for \(\times\) is 1.
...
\begin{frame}{Why is the product of negative numbers positive?}


  \begin{myth} Let \(({\mathcal F},  +, \times)\) be a field. For all \(a,b \in \mathcal{F}\), we have
\begin{alphalist}
  \item \(0 a = a 0 = 0\).
  \item \(a (-b) = (-a) b = -(a b) \)
    \item \(- (-a) = a \)
...
The last is the most mysterious for grade school students. A proof follows from replacing \(a \to -a\) in  \(a (-b) = (-a) b\) and using \(-(-a) = a\).

\end{frame}
\begin{frame}
Let's prove \(a (-b) = (-a) b = -(a b) \).
\begin{myproof} Let \(({\mathcal F},  +, \times)\) be a field. For all
  \(a,b \in {\mathcal F}\), we have
  \[
   0 = a 0 = a (b + -b) = a b + a (-b).
  \]
  So \(-(ab) = a (-b)\). Interchanging \(a\) and \(b\) and using commutivity gives
   \(-(ab) = (-a) b\).
...
\item \( [b - a \in P ] \equiv [a < b]. \)

\item \( [a \leq b ] \equiv [a < b \lor a = b]. \)
...
\begin{frame}{Assume the reals}
We will assume that there is a set \(\reals\) with members called \emph{real numbers} along with functions \(+ : \reals \times \reals \to \reals\) and \(\times : \reals \times \reals \to \reals\). Further we'll assume that this field is ordered by the positive numbers.
  \end{frame}

  \begin{frame}

    \begin{myth} Let \(({\mathcal F},  +, \times)\) be an ordered field. For all
