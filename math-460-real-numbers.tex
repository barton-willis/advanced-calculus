...
\item Addition \(+\) is a binary operator on \(\reals\).  Since \(x +0 = x\) for all real \(x\), the identity element for addition is zero.  Further we know that addition is
commutative and associative.

\item Function composition \(\circ \) is a binary operator on the set of functions from \(\reals\) to \(\reals\).  The function \(x \in \reals \mapsto x\) is the identity element for
...
\begin{myth} Let \( S \) be a set and let \(F\) be a commutative binary operator on \(S\).  Then \(F\) has at most one identity element.
 ...
 \begin{myproof} Let \(\theta\) and \(\theta^\prime\) be identity elements for \(F\). We'll show that \(\theta = \theta^\prime\).  We have
 \begin{align*}
